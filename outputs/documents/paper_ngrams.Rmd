...
We employ n-gram models to calculate consistency scores and generate word candidates for text corrections. The n-gram approach involves identifying words that are commonly found together. In our application, we use these sequences to calculate consistency scores and generate word candidates for text corrections. For instance, if using a sequence of three words, or tri-grams, then we identify the first two words in the sequence and evaluate if the last word in the sequence is 'as expected' by comparing it to the corresponding tri-gram in a comparison dataset. The sequences we employed are a mixture of five, four and three consecutive words also known as 5-grams, 4-grams and tri-grams.
...
In our application, we use a model that has a mixture of 5-grams, 4-grams and tri-grams to provide word replacements to a text error. Longer n-grams such as 5-grams capture more context of word sequences and potentially generate more accurate predictions. However, there is less probability for a longer sequence of words to fit a specific combination and leads to data sparsity. Therefore, shorter n-grams should still be included to preserve generalizability of the predictions.
...
In the 4-gram model, one extra more word "elephant" is included and thus provides more context for the word "room" to be accurately predicted. However, there are times people would simply describe the location of an elephant; for example, "elephant in the pond". The combination of this sequence as a 4-gram token might not be in a comparison dataset and no prediction would be available if only the 4-gram models are used. Therefore, we have to also rely on the tri-gram models to provide other word candidates including "in the pond".
...
Finally, the `make_internal_consistency_dataset` function splits the n-grams to two parts: the previous $n-1$ words and the last word. As a result, a tibble is created with each n-gram containing its original sequence, the first words, and the last word:
