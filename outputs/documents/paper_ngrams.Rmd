...
title: "aRianna: R package for consistency scores in text data"
...
We employ n-gram models to calculate consistency scores and generate word candidates for text corrections. The n-gram approach involves identifying two, three, or more, words that are commonly found together. In our application, we use the sequence of three words, or tri-grams, to calculate consistency scores and generate word candidates for text corrections. By using tri-grams, we identify the first two words in the sequence and evaluate if the last word in the sequence is "as expected" by comparing it to the corresponding tri-gram in a comparison dataset. The comparison dataset can be internal, meaning that the dataset is the source where the text being evaluated is extracted from. On the other hand, a comparison dataset is external when the dataset is from a different source than where the text being evaluated is from, and is a larger and more universal dataset that holds generalizability. The external dataset we use in the application is constructed using a subset of text data sourced from Open WebText Corpus by @Gokaslan2019OpenWeb. Since the content of Open WebText Corpus is from the web, the variety of the text data is promising for meeting the generalizability and universality an external dataset should possess. 

In this paper, we introduce the `aRianna` R Package with the following structure: In Section 2, we provide an overview of language model and n-grams. In section we deconstruct the package functions and provide demonstration example of the package usage. Finally, we discuss the limitation and the future directions of aRianna in Section 4. Beyond this paper, we construct a Shiny app available at: https://kelichiu.shinyapps.io/aRianna/. That app computes internal and external consistency scores for corpus excerpts, and have developed an R Package, `aRianna`, that allows our approach to be used on larger datasets, which is available at: https://github.com/RohanAlexander/aRianna.
...
## Function to Generate Internal Consistency Score 
...
## Function to Generate External Consistency Score
