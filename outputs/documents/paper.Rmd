...
## Word Embedding Models: GloVe and Word2Vec

**Article: GloVe: Global Vectors for Word Representation**

GloVe is a pre-trained word embedding model that can capture semantic meanings of words. It is an unsupervised learning algorithm developed by Stanford. Their disadvantage is that they are context-free and fail to capture higher-level concepts in context.

**Article: word2vec Explained: Deriving Mikolov et al.’s Negative-Sampling Word-Embedding Method**

Developed by Google, Word2Vec is another word embedding model that uses shallow neural network. It's not for deep learning.
...
GPT-2 has the capacity to adapt to a context and generates custom output with the "fine-tuning" technique **(Ke-Li add ref)**. While the pre-training is unsupervised, fine-tuning is conducted by supervised learning or reinforced learning (@ziegler2019fine). **(Ke-Li add more about fine-tuning)** We aim to use GPT-2 to clean textual data to **(Ke-Li add more about how we use fine-tuning to train GPT-2 for text cleaning)**
