...
There are more than one way to implement GloVe word embedding model in R. One way is to load GloVe embeddings into a frozen Keras Embedding layer **(Keli add citation to the blog post by Keras)** using the Keras R package (**(Keli add citation to package)**). Another way, as mentioned and briefly demonstrated by Rstudio in their blog post, is to use the "text2vec" package developed by Dmitriy Selivanov **(Ke-Li add package citation)**. 
