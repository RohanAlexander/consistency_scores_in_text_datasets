...
Here, we follow @falbel2017word to implement a skip-gram model of word2vec through Keras. There are three steps in the Keras deep learning framework: 1) prepare the data; 2) define the model; and 3) train and evaluate the model. **(Ke-Li: Can we change this to be a different dataset please?)** Following these steps, we first downloaded the **(Ke-Li: Is there a reason this is in brackets?)** [Amazon Fine Foods Reviews dataset from Stanford] [@amzfinefooddata] and preprocess the text into tokens. 
...
There are a variety of ways to implement GloVe word embedding models in R. One is to load GloVe embeddings into a frozen Keras Embedding layer [@kerasglove] using the `Keras` R package [@kerasR2020]. This means **(Ke-Li: Add what it means please.)** Another way, is to use the `text2vec` package [@text2vec]. 

Here we follow **CITE** to implement GloVe using the `keras` R package [@kerasglove]. The blog post is aimed to create a word embedding model for text classification. However, we will only use the guide create a embedding index and skip the steps of text classification. Instead of building a model for word embedding from scratch as we did for the skip-gram model of Word2Vec, here we are merely loading the Glove model and its pre-trained word vectors as an index. The word embedding index is then saved to an R environment.


**[As before, we need code and examples here]**
