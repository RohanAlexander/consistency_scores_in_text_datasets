...
Other Word2Vec use cases:
1. Find the most similar words (**Keli add example**)
2. Calculate the similarities between a pair of words (**Keli add example**)
3. Find the odd word in a list of words (**Keli add example**)
...
The ratio between the co-occurrence probability of "ice, solid" pair and "steam, solid" pair is high because the co-occurrence of "ice" and "solid" is higher than the co-occurrence of "steam" and "solid". Reversely, the ratio between the co-occurrence of "ice, gas" pair and "steam, gas" pair is low. Other other hand, the ratio of co-occurrence probability of "ice, water" pair and "steam, water" pair are close to 1 because water is relevant to both "ice" and "steam". Probability alone does not distinguish well relevant words from irrelevant words. For this reason, the researcher of GloVe argued that "the appropriate starting point for word vector learning should be with ratios of co-occurrence probabilities rather than the probabilities themselves (@pennington2014glove)."
...
Because BERT employs the Transformer architecture with the self-attention mechanism, fine-tuning is also available.
