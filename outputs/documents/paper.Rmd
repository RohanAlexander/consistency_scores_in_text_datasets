...
Language models underpinned by n-grams are widely applied in text prediction, spelling-correction, and machine translation **(Ke-Li add citation)**. In our application, we use a tri-gram based model to detect both real-word and non-word errors and correct them with the candidate word that has the highest probability. **(TODO: Add more about how we use n-gram once the functions in the app are in place.)**
